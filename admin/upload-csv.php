...
<ol>
    ...
    <li>Download the above linked sample file to your computer. Note the format of the values under each column. Your data will be required to be in the same format. No typos are acceptable. In the samples below, quote marks are used to delineate acceptable values; do not use the quote marks when typing in the file unless they are part of the question's question or answer. For reference, the accepted values for each column are:
        <ul class="browser-default">
            ...
            <li><b>Points</b>: Number of points for the question. Should be a number like 32 and not "thirty-two".</li>
            ...
            <li><b>Start Chapter</b>: Chapter number for the question's starting verse. Should be a number like 32 and not "thirty-two". Must match a chapter already in the system for the given Bible book.</li>
            <li><b>Start Verse</b>: Verse number for the question's starting verse. Should be a number like 32 and not "thirty-two". Must be greater than 0 and less than or equal to the number of verses for the given Bible book and chapter already in the system.</li>
    ...
    <li>Type in as many questions into the upload form as you like. Remember to avoid deleting the header row! <b>Do not type any commas into any of the columns. If you must add a comma to a question/answer, you must do so on this website and not via the upload form.</b></li>
    <li>When you're ready to upload the file, choose "File" -> "Save As" from the menu. Save the document as a CSV (Comma delimited) CSV file. <b>If you see an option for CSV UTF-8, do not choose this option. Choose the "CSV (Comma delimited)" on Windows or the "Comma Separated Values (.csv)" otion on macOS.</b> Save the file in a location you can find because you'll need it in the next step. If Excel warns you that some features of the worksheet may be lost, just say OK.</li>
